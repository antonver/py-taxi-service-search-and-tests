...
class CarTests(TestCase):
    ...
    def setUpTestData(cls):
        manufacturer = Manufacturer.objects.create(name="Mercedes", country="Ukraine")
        for i in range(10):
            Car.objects.create(model=f"Car_{i}", manufacturer=manufacturer)
    ...
    def test_search_form_with_one_result(self):
        response = self.client.get(reverse("taxi:car-list"), {"model": "Car_0"})
        self.assertEqual(response.status_code, 200)
    ...
    def tes_search_form_with_zero_result(self):
        response = self.client.get(reverse("taxi:car-list"), {"model": "Pampam"})
        self.assertEqual(response.status_code, 200)
        self.assertIn("car_list", response.context)
    ...
    def test_search_form_all_results_second_page(self):
        response = self.client.get(reverse("taxi:car-list"), {"model": "", "page": 2})
        self.assertEqual(response.status_code, 200)
        self.assertIn("car_list", response.context)
        self.assertEqual(len(response.context["car_list"]), 5)
